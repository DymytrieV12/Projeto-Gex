...
class _CarrinhoScreenState extends State<CarrinhoScreen> {
  ...
  bool _panelExpanded = false; // Painel compactado por padrão
  ...
  Future<void> _loadOpcoes() async {
    ...
    final results = await Future.wait([ApiService.getFormasPagamento(), ApiService.getTiposEntrega(), ApiService.getDescontoProgressivo()]);
    if (!mounted) return;
    setState(() {
  ...
  Future<void> _finalizarPedido() async {
    ...
    final total = (subtotal - valDesc) + _valorFrete;
    ...
    final r = await ApiService.criarPedido(formaPagamentoId: _formaSelecionada!.id, tipoEntrega: _entregaSelecionada!.id, quantidadeParcela: _parcelas, valorTotal: total, produtos: produtos, valorFrete: _valorFrete > 0 ? _valorFrete : null, observacao: obs, valorGreenCash: 0);
    if (!mounted) return;
    setState(() => _finalizando = false);
    if (r['success'] == true) {
      cart.limpar(); _obsC.clear();
      showDialog(context: context, barrierDismissible: false, builder: (ctx) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (pct > 0) ...[const SizedBox(height: 8), Text('Desconto aplicado: ${pct.toInt()}%', style: const TextStyle(color: Color(0xFF0E5A35), fontWeight: FontWeight.bold))],
        ]),
  ...
  Widget build(BuildContext context) {
    ...
    final total = subtotalDesc + _valorFrete;
    ...
    return Scaffold(
      ...
      body: cart.itens.isEmpty
          ...
          : Column(children: [
              ...
              // Lista de itens — expande para ocupar espaço disponível
              Expanded(
              ...
              AnimatedContainer(
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  // Handle para expandir/compactar
                  GestureDetector(
