...
class _CarrinhoScreenState extends State<CarrinhoScreen> {
  ...
  double _valorGreenCash = 0;
  bool _usarGreenCash = false;
  final _obsC = TextEditingController();

  // IDs permitidos
  static const _entregasPermitidas = {'Moto Taxi', 'Moto Táxi', 'Sedex', 'PAC', 'Enviar para o cliente', 'Receber em casa'};
  static const _pgtoPermitidos = {1, 3}; // Dinheiro=1, Boleto/PIX=3
  ...
  Future<void> _loadOpcoes() async {
    ...
    setState(() {
      if (results[0]['success'] == true) {
        final all = (results[0]['data'] as List)
            .map((j) => FormaPagamento.fromJson(j))
            .toList();
        // Filter: Dinheiro (1), Boleto/PIX (3) only
      ...
      if (results[1]['success'] == true) {
        final all = (results[1]['data'] as List)
            .map((j) => TipoEntrega.fromJson(j))
            .toList();
        // Filter: only Moto Taxi, Sedex-like, PAC-like
        _tiposEntrega = all.where((t) {
          final desc = t.descricao.toLowerCase();
          return desc.contains('moto') ||
              desc.contains('sedex') ||
              desc.contains('pac') ||
              desc.contains('enviar') ||
              desc.contains('receber em casa');
        }).toList();
        if (_tiposEntrega.isEmpty) _tiposEntrega = all; // fallback
  ...
  Future<void> _calcularFrete() async {
    ...
    final r = await ApiService.calcularFrete(
      tipoEntrega: _entregaSelecionada!.id,
      valorTotal: cart.totalValor,
    );
    ...
    setState(() {
      _calcFrete = false;
      if (r['success'] == true) {
        _valorFrete = (r['data']?['valorFrete'] as num?)?.toDouble() ?? 0;
      }
    });
  }
  ...
  Future<void> _finalizarPedido() async {
    ...
    final total = cart.totalValor + _valorFrete;
    ...
    final r = await ApiService.criarPedido(
      ...
      observacao: obs,
      valorGreenCash: _usarGreenCash ? _valorGreenCash : 0,
    );
    ...
    if (r['success'] == true) {
      ...
      showDialog(
        ...
        builder: (ctx) => AlertDialog(
          ...
          content: Text('Pedido #${r['data']?.toString() ?? ''} registrado com sucesso.'),
          actions: [
            ElevatedButton(
              onPressed: () {
                Navigator.pop(ctx);
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Erro: ${r['error'] ?? 'Desconhecido'}'),
                              ...
                              children: [
                                Container(
                                  ...
                                  child: item.imagemUrl != null
                                      ? ClipRRect(
                                          borderRadius: BorderRadius.circular(10),
                                          child: Image.network(item.imagemUrl!, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.eco, color: Color(0xFF0E5A35))),
                                        )
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      Text('Sub: R\$ ${item.subtotal.toStringAsFixed(2)}', style: const TextStyle(color: Color(0xFF0E5A35), fontWeight: FontWeight.bold, fontSize: 13)),
                                    ],
                                  ),
                                ),
                                Column(
                                  children: [
                                    Container(
                                      decoration: BoxDecoration(
                                        color: const Color(0xFF0E5A35).withValues(alpha: 0.08),
                                        borderRadius: BorderRadius.circular(10),
                                      ),
                                      child: Row(
                                    ...
                                    IconButton(
                                      ...
                                      onPressed: () => cart.removeItem(item.produtoId),
                                    ),
                                  ],
                                ),
                              ],
                          ...
                          Row(
                            ...
                            children: [
                              ...
                              _calcFrete
                                  ...
                                  : Text(
                                      _valorFrete == 0 ? 'Gratis' : 'R\$ ${_valorFrete.toStringAsFixed(2)}',
                                      style: TextStyle(color: _valorFrete == 0 ? const Color(0xFF0E5A35) : Colors.black87, fontSize: 13, fontWeight: FontWeight.w600),
                                    ),
                            ],
                          ),
