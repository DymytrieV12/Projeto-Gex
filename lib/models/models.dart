class Produto {
  ...
  factory Produto.fromJson(Map<String, dynamic> json) {
    ...
    return Produto(
      ...
      quantidadeMinima: _parseInt(json['quantidadeMinimaVenda'], fallback: 1),
      quantidadeMaxima: json['quantidadeMaximaVenda'] == null
          ? null
          : _parseInt(json['quantidadeMaximaVenda']),
    );
  }
}
...
class Pedido {
  ...
  final String? imagemQrCodePix;
  final String? paginaPix;
  final String? codigoBarrasBoleto;
  final String? pdfBoleto;
  ...
  bool get temPix =>
      _hasText(qrCodePix) || _hasText(imagemQrCodePix) || _hasText(paginaPix);

  bool get temBoleto =>
      _hasText(codigoBarrasBoleto) || _hasText(pdfBoleto);

  bool get temDadosPagamento => temPix || temBoleto || _hasText(linkPagamento);
  ...
  factory Pedido.fromJson(Map<String, dynamic> json) {
    ...
    final pixMap = _deepFindMap(json, 'pix');
    final boletoMap = _deepFindMap(json, 'boleto');

    final qrCodePix = _firstNonEmptyString([
      pixMap?['qrCode'],
      pixMap?['copyPaste'],
      pixMap?['copiaECola'],
      json['qrCode'],
      json['pixQrCode'],
      json['codigoPix'],
      json['codigoPixCopiaCola'],
      json['pixCopiaECola'],
    ]);

    final imagemQrCodePix = _firstNonEmptyString([
      pixMap?['image'],
      json['pixImage'],
      json['imagemQrCodePix'],
      json['imagemPix'],
      json['qrCodeImage'],
    ]);

    final paginaPix = _firstNonEmptyString([
      pixMap?['page'],
      json['pixPage'],
      json['paginaPix'],
      json['paymentLink'],
    ...
    final codigoBarrasBoleto = _firstNonEmptyString([
      boletoMap?['bankLine'],
      boletoMap?['linhaDigitavel'],
      json['bankLine'],
      json['linhaDigitavel'],
      json['codigoBarrasBoleto'],
      json['codigoBarraBoleto'],
    ]);

    final pdfBoleto = _firstNonEmptyString([
      boletoMap?['pdf'],
      json['boletoPdf'],
      json['pdfBoleto'],
      _deepFindValue(json, 'pdf'),
    ]);
    ...
    return Pedido(
      ...
      numeroPedido: (json['numeroPedido'] ?? json['numero'] ?? json['idPedido'])
          ?.toString(),
      ...
      status: json['statusPedido']?.toString() ??
      ...
      quantidadeParcelas: _parseInt(json['quantidadeParcelas'], fallback: 1),
      observacao: json['observacao']?.toString(),
      codigoRastreamento: json['codigoRastreamento']?.toString(),
      linkPagamento: linkPagamento,
      qrCodePix: qrCodePix,
      imagemQrCodePix: imagemQrCodePix,
      paginaPix: paginaPix,
...
class ItemPedido {
  ...
  factory ItemPedido.fromJson(Map<String, dynamic> json) {
    final quantidade = _parseInt(json['quantidade'] ?? json['qtd'], fallback: 1);
...
class GreenCash {
  ...
  final double cashPadrao;
  final double cashBonusEquipe;
  final double cashBonusPessoal;
  final double saldoDisponivel;
  final double saldoBloqueado;

  GreenCash({
    required this.nomeGraduacao,
    required this.cashPadrao,
    required this.cashBonusEquipe,
    required this.cashBonusPessoal,
    required this.saldoDisponivel,
    required this.saldoBloqueado,
  });

  factory GreenCash.fromJson(Map<String, dynamic> j) => GreenCash(
        nomeGraduacao: j['nomeGraduacao']?.toString() ?? '',
        cashPadrao: _parseDouble(j['cashPadrao']),
        cashBonusEquipe: _parseDouble(j['cashBonusEquipe']),
        cashBonusPessoal: _parseDouble(j['cashBonusPessoal']),
        saldoDisponivel: _parseDouble(j['saldoDisponivel']),
        saldoBloqueado: _parseDouble(j['saldoBloqueado']),
      );
}
...
class ItemCarrinho {
  ...
  factory ItemCarrinho.fromJson(Map<String, dynamic> json) {
    return ItemCarrinho(
      ...
      quantidade: _parseInt(json['quantidade'], fallback: 1),
    );
  }
}
...
String? _firstNonEmptyString(List<dynamic> values) {
  for (final value in values) {
    ...
    if (text != null && text.trim().isNotEmpty && text.trim().toLowerCase() != 'null') {
      return text.trim();
    }
  }
  return null;
...
dynamic _deepFindValue(dynamic root, String wantedKey) {
  if (root is Map) {
    for (final entry in root.entries) {
      if (entry.key.toString().toLowerCase() == wantedKey.toLowerCase()) {
        return entry.value;
      }
    }
    for (final value in root.values) {
      final found = _deepFindValue(value, wantedKey);
      if (found != null) return found;
    }
  }
  if (root is List) {
    for (final item in root) {
      final found = _deepFindValue(item, wantedKey);
      if (found != null) return found;
    }
  }
  return null;
...
Map<String, dynamic>? _deepFindMap(dynamic root, String wantedKey) {
  return _asMap(_deepFindValue(root, wantedKey));
}
